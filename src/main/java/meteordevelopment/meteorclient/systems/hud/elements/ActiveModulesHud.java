...
public class ActiveModulesHud extends HudElement {
    ...
    private final Setting<Boolean> randomFade = sgGeneral.add(new BoolSetting.Builder()
        .name("randomFade")
    ...
    private final Setting<SettingColor> flatColor = sgGeneral.add(new ColorSetting.Builder()
        ...
        .visible(() -> colorMode.get() == ColorMode.Flat || (colorMode.get().equals(ColorMode.Fade) && !randomFade.get()))
        .build()
    );
    ...
    private void renderModule(HudRenderer renderer, List<Module> modules, int index, double x, double y) {
        ...
        switch (colorMode.get()) {
            ...
            case Fade -> {
                ...
                // Determine base color based on flatColor or module.color
                java.awt.Color baseColor;
                if (randomFade.get()) {
                    baseColor = new java.awt.Color(module.color.r, module.color.g, module.color.b);
                } else {
                    baseColor = new java.awt.Color(flatColor.get().r, flatColor.get().g, flatColor.get().b);
                }

                // Calculate brightness based on the fade logic
                float brightness = MathHelper.abs(MathHelper.sin(System.currentTimeMillis() % 6000L / 6000.0F * (float) Math.PI)) + 0.1f;
                brightness *= 1.3f;
                brightness += 0.5f;
                if (brightness < 0.2f) brightness = 0.2f;
                else if (brightness > 1.0F) brightness = 1.0F;

                // Apply adjusted brightness to the original color
                int finalRed = (int) (baseColor.getRed() * brightness);
                int finalGreen = (int) (baseColor.getGreen() * brightness);
                int finalBlue = (int) (baseColor.getBlue() * brightness);

                // Ensure final color components are within [0, 255]
                finalRed = Math.min(Math.max(finalRed, 0), 255);
                finalGreen = Math.min(Math.max(finalGreen, 0), 255);
                finalBlue = Math.min(Math.max(finalBlue, 0), 255);

                java.awt.Color finalColor = new java.awt.Color(finalRed, finalGreen, finalBlue);
